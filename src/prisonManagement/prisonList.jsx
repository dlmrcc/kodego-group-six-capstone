...
import { AddPrisonAPI, DeletePrisonAPI, GetPrisonListAPI, UpdatePrisonAPI } from '../Utils/API/prisonManagement';
...
function PrisonList() {
  ...
  const [prisonStatus, setPrisonStatus] = useState(1);
  ...
  //Load the prison list to table
  const fetchData = () => {
    GetPrisonListAPI(prisonStatus)
      .then((result) => {
        console.log(result)
        return result.json()
      })
  ...
  const handleSaveButton = () => {
    const name = formatString(document.getElementById('prisonName').value)
    if (addButton === "Save") {
      //Save New API
      if (name == '') {
        alert("The prison name field is empty.")
      }
      else {
        ...
        AddPrisonAPI(name)
          ...
          .then((result) => {
            if (result.success === false) {
              ...
              alert("Succesfully added new prison")
              setShowAddForm(false)
              fetchData();
            }
          })
        ...
        UpdatePrisonAPI(name, idUpdate)
          ...
          .then((result) => {
            if (result.success === false) {
              ...
              alert("Succesfully update the prison")
              setShowAddForm(false)
              fetchData();
            }
          })
    ...
    <div className="flex justify-center items-center ">
      <div className="bg-white rounded-lg shadow-md p-4 mt-6 mb-5 ">
          ...
          <div>
            ...
            <div className="mr-0 flex ">
              ...
              <input
                id="prisonName"
                type="text"
                placeholder="Type here new prison name"
                value={inputValue}
        ...
        <div className="flex justify-between items-center mt-2 mb-4">
          <h2 className="font-medium">List of Prison</h2>
        </div>
        <div>
